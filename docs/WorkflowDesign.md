...
|----------|---------------|
| **JobEscrow.sol** | Job creation, milestone management, escrow locking/release, configurable timeouts, cancellation. **Serves as the single authoritative source of fund and milestone state.** All state-mutating functions enforce a **state mutex** (per-milestone status check-before-update) to prevent race conditions between competing calls (e.g., `approveMilestone()` vs. `triggerAutoApprove()` vs. `raiseDispute()` in the same block). All terminal operations are **idempotent** — repeated calls after the first successful execution are no-ops. |
| **Dispute.sol** | Dispute creation, evidence submission, recording the platform judge's ruling. **Does not hold or directly transfer funds.** When a ruling is executed, `Dispute.sol` calls a restricted interface on `JobEscrow.sol` (e.g., `executeDisputeRuling(jobId, milestoneId, ruling)`) to redistribute funds. This ensures `JobEscrow.sol` remains the single point of fund custody and state authority, preventing cross-contract inconsistencies. |
| **Reputation.sol** | Soulbound (non-transferable) on-chain reputation scores for both clients and freelancers. Reputation updates are triggered by `JobEscrow.sol` upon milestone completion or dispute conclusion via a restricted callback, ensuring reputation cannot be updated without a corresponding fund state change. |
| **DataAvailability.sol** | On-chain CID registry, pinning confirmation records, retention period enforcement. Emits `CIDRegistered` events consumed by the platform's IPFS pinning node to guarantee data availability. |
| **USDC** | External stablecoin used for all payments |
...
|------|-------|--------|---------|
| **1** | Client | `postJob()` | Locks 100% of job value (USDC) in escrow. **Chooses the review timeout** $T_{\text{review}}$ from the allowed set: **{1 day, 3 days, 7 days, 14 days, 21 days, 30 days}**. This timeout is stored on-chain and **cannot be changed** after posting. Generates per-job symmetric key $K_{job}$ and a **cryptographically random 256-bit salt**. Encrypts the agreement (including the salt) with $K_{job}$, uploads ciphertext to IPFS, registers the IPFS CID on-chain via `DataAvailability.sol` (emitting a `CIDRegistered` event that triggers the platform pinning node to auto-pin the content), and stores `agreementHash = keccak256(salt ‖ plaintext)` on-chain. The salt is embedded inside the encrypted IPFS payload — it is never published in the clear. This prevents **confirmation attacks** where an adversary who suspects the agreement content (e.g., from a standard template) could brute-force the hash to verify their guess. Clients also lock a **graduated behavior bond** based on their reputation tier: 7.5% (New), 5% (Bronze), 2.5% (Silver), or 1% (Gold). |
| **2** | Freelancer(s) | `applyForJob()` | One or more freelancers browse open jobs. **The chosen $T_{\text{review}}$ is publicly visible on-chain**, so freelancers know exactly how long the client has to review before auto-approval kicks in. They can factor this into their decision to apply. Each application includes an optional IPFS proposal hash and their on-chain reputation is readable from `Reputation.sol`. No deposit or commitment is required to apply. |
| **3** | Client | `selectFreelancer()` | Client reviews applications (reputation scores, proposal content) and selects one freelancer. Publishes $\text{Enc}(pk_{\text{freelancer}}, K_{job})$ on-chain so the freelancer can decrypt all job content. Starts the $T_{\text{stake}}$ timer (3 days). The freelancer may **accept** (Step 4a), **explicitly reject** (Step 4b), or **let the offer expire** (auto-reject on $T_{\text{stake}}$ expiry). In all rejection cases, the client can immediately select another applicant via `reselectFreelancer()`. |
| **4a** | Freelancer | `confirmAndStake()` | Decrypts $K_{job}$ using their private key. Reviews the full agreement. If they agree, stakes 5% deposit in USDC. Job transitions to **Active**. Must be called within $T_{\text{stake}}$ (3 days). |
...
| **5** | Freelancer | `submitMilestone()` | Encrypts deliverable with $K_{job}$, uploads to IPFS, registers the deliverable CID on-chain via `DataAvailability.sol` (platform pinning node auto-pins upon `CIDRegistered` event), and records ciphertext hash on-chain. The review timer ($T_{\text{review}}$, as chosen by the client at Step 1) starts counting. |
...
| **7** | Contract | Funds released | Milestone USDC (minus 2% protocol fee) is credited to the freelancer's withdrawable balance. |
| **8** | Contract | Final release | After the last milestone is approved: freelancer's 5% deposit is refunded; client's behavior bond (graduated by tier) is refunded; reputation is updated for both parties. |
...
| **$T_{\text{deadline}}$** | Per-milestone (set by client) | If freelancer misses deadline, client can call `claimAbandonment()` → deposit forfeited, escrow returned |
...
| **APPLICATIONS** (freelancer selected, awaiting `confirmAndStake()`) | Client | `cancelJob()` | 100% of escrowed funds returned to client. Behavior bond refunded in full. | Client incurs a minor cancellation penalty ($C \times 0.1$ in the reputation formula). The selected freelancer's reputation is unaffected. |
...
| **ACTIVE** (freelancer staked, work in progress, no milestone submitted yet) | Client | `requestCancellation()` | Requires **freelancer consent** via `acceptCancellation()`. If both agree: remaining escrow returned to client; freelancer's deposit refunded in full; funds for any already-approved milestones are unaffected (already released). | Client incurs a cancellation penalty ($C \times 0.1$). Freelancer reputation unaffected. |
| **ACTIVE** | Freelancer | `requestCancellation()` | Requires **client consent** via `acceptCancellation()`. If both agree: remaining escrow returned to client; freelancer's deposit refunded in full. | Freelancer incurs a minor reputation penalty (equivalent to an incomplete job). Client reputation unaffected. |
| **ACTIVE** | Either party (unilateral, no consent) | `raiseDispute()` | **Unilateral cancellation is not permitted in ACTIVE state.** The dissatisfied party must raise a dispute, which follows the dispute resolution path (§4). This prevents a client from pulling funds after the freelancer has committed, or a freelancer from walking away without consequence. | N/A — routed to dispute. |
...
2. **Unilateral cancellation is only possible before work begins** (OPEN or APPLICATIONS states). Once the freelancer has staked their deposit and the job is ACTIVE, unilateral exit requires the dispute path.
...
4. **$T_{\text{acceptance}}$ auto-cancellation.** If the job remains in OPEN or APPLICATIONS state for longer than $T_{\text{acceptance}}$ (14 days) without a freelancer being selected and confirmed, the client can withdraw all escrowed funds — effectively an automatic cancellation with no penalty.
...
| Step | Actor | Action | Details |
|------|-------|--------|---------|
| **1** | Client or Freelancer | `raiseDispute()` | The disputing party pays a **dispute fee** calculated as $F_{\text{dispute}} = \max(F_{\text{base}},\; \min(1\% \times V_{\text{milestone}},\; F_{\text{cap}}))$, where $F_{\text{base}} = 10$ USDC and $F_{\text{cap}} = 1{,}000$ USDC. By denominating the fee in the same stablecoin (USDC) used for all payments, the fee's real value is deterministic and immune to crypto-asset volatility — neither party faces unpredictable costs due to ETH price swings. This scaled formula keeps the fee proportional to stake — affordable for small jobs while meaningful enough to deter frivolous disputes on large ones. The disputed milestone's funds are **frozen in escrow** and the **automated release timer ($T_{\text{review}}$) is paused**. No auto-approve can be triggered while the dispute is active. The timer remains paused until the platform signals a manual resolution via `executeRuling()`. The milestone state changes to **Disputed**. Calls `Dispute.createDispute()`. |
| **2** | Both parties | `submitEvidence()` | Both parties have a **5-day evidence window** ($T_{\text{evidence}}$) to submit evidence. Evidence is encrypted with $K_{job}$ and uploaded to IPFS; the CID is registered on-chain via `DataAvailability.sol` (platform pinning node auto-pins upon `CIDRegistered` event), and the ciphertext hash is recorded on-chain with a block timestamp. Evidence follows the **ERC-1497 (Evidence Standard)** format. |
| **3** | Platform | Assign judge | The platform assigns one or more judges with relevant domain expertise. For each dispute, the assigned judge generates a **fresh ephemeral keypair** $(pk_{\text{judge}}^{\text{eph}}, sk_{\text{judge}}^{\text{eph}})$ specific to this dispute. The ephemeral public key $pk_{\text{judge}}^{\text{eph}}$ is published on-chain (emitted in a `JudgeAssigned(disputeId, judgeAddress, ephemeralPubKey)` event). This ensures that even if a judge's long-term identity key is compromised, no historical dispute content is exposed — each dispute's decryption capability is isolated to its ephemeral key. The judge's identity address is also recorded for accountability. |
| **4** | Both parties | `distributeKeyToJudge()` | Both the client and freelancer encrypt $K_{job}$ with the judge's **ephemeral public key** and submit it on-chain: $\text{Enc}(pk_{\text{judge}}^{\text{eph}}, K_{job})$. Must be done within $T_{\text{keyDistribution}}$ (2 days). Non-cooperation triggers a default ruling against the non-cooperating party (same rules as the full design — see §4.5). |
...
| **6** | Judge | `submitRuling()` | The judge submits a ruling on-chain. The ruling is one of three outcomes (see §4.4). A hash of the written reasoning is also recorded on-chain for transparency. |
| **7** | Contract | `executeRuling()` | Funds are distributed per the ruling. Reputation is updated. Deposits and behavior bonds are handled according to the outcome. Upon ruling execution, the judge is required to **delete the ephemeral private key** $sk_{\text{judge}}^{\text{eph}}$ and any cached plaintext. Only the ruling hash and on-chain records are retained for auditability. |

### 3.4 Ruling Outcomes

| Ruling | Effect on Funds | Effect on Deposits & Bonds | Effect on Reputation |
|--------|----------------|---------------------------|---------------------|
| **1 — Freelancer Wins** | Milestone funds released to freelancer (minus 2% fee) | Freelancer deposit untouched. Client loses dispute fee. Up to 3% of milestone value is slashed from the client's behavior bond and **sent to the platform treasury** (not to the freelancer). | Client: lost-dispute penalty ($L \times 0.3$). Freelancer: milestone credited at 0.5× multiplier (dispute win). |
| **2 — Client Wins** | Milestone funds returned to client | Freelancer's deposit may be partially slashed (up to 50% of the disputed milestone's proportional deposit); **slashed amount goes to the platform treasury**. Client's dispute fee refunded. Behavior bond untouched. | Freelancer: lost-dispute penalty ($L \times 0.3$). Client: no penalty. |
| **0 — Inconclusive** | Funds split proportionally (e.g., 50/50) or returned to escrow pending mutual resolution | Both deposits untouched. Neither dispute fee refunded. | No reputation penalty for either party. |

### 3.5 Timeout & Non-Cooperation Rules

| Timeout | Duration | Effect |
|---------|----------|--------|
| **$T_{\text{evidence}}$** | 5 days | Both parties submit evidence. Late evidence is ignored. |
...
| **$T_{\text{ruling}}$** | 14 days (after key distribution) | Judge must submit ruling. If judge fails to rule, the platform must reassign. |

**Key non-cooperation handling (unchanged from full design):**

| Scenario | Consequence |
|----------|-------------|
| Client fails to submit key within $T_{\text{keyDistribution}}$ | Default ruling in favor of freelancer |
| Freelancer fails to submit key within $T_{\text{keyDistribution}}$ | Default ruling in favor of client |
| Both parties fail to submit keys | Ruling = 0 (Inconclusive); funds split proportionally |
| A party submits an incorrect key (judge decrypts but $\text{keccak256}(\text{salt} \| \text{result}) \neq \text{agreementHash}$, where the salt is extracted from the decrypted payload) | Treated as non-cooperation — same as failing to submit |
...
```
    ...
    └────────┬─────────┘
             ...
             ▼
    ┌──────────────────┐
    │ OFFER_PENDING    │  Freelancer has T_stake to respond
    └─┬──────┬──────┬───┘
     │      │      │
...
| **Freelancer Deposit** | 5% of contract value (USDC) | Skin-in-the-game. Deters abandonment and garbage delivery. Forfeited on abandonment. **Any deposit slashing goes to the platform treasury**. |
| **Dispute Fee** | Scaled: $\max(10\text{ USDC},\; \min(1\% \times V_{\text{milestone}},\; 1{,}000\text{ USDC}))$ | Cost to initiate a dispute. Denominated in USDC to eliminate volatility risk — the fee's real value is deterministic regardless of crypto market conditions. Scales with milestone value to remain proportional — affordable for small jobs, meaningful for large ones. Refunded to the winner. |
...
| **Dispute Fee** | Scaled: $\max(10\text{ USDC},\; \min(1\% \times V_{\text{milestone}},\; 1{,}000\text{ USDC}))$ | Paid by the party raising the dispute in USDC. Refunded if they win. Scales with milestone value to avoid over-deterring small disputes or under-deterring large frivolous ones. Using USDC ensures fee predictability — no oracle dependency or ETH price exposure. |
...
- Their 5% deposit is at stake — abandoning or delivering garbage means losing it.
...
- $F_{\text{dispute}} = \max(10\text{ USDC},\; \min(1\% \times V_{\text{milestone}},\; 1{,}000\text{ USDC}))$ — scales with milestone value, denominated in the same stablecoin as all payments
- $B_{\text{bond\_slash}}$ = behavior bond penalty (graduated by tier: 7.5% New, 5% Bronze, 2.5% Silver, 1% Gold — slashed to treasury)

The dispute fee is designed to **scale with the value at stake**: for a $500 milestone, the fee is $10 USDC (the floor); for a $50,000 milestone, the fee is $500 USDC (1%); for a $200,000 milestone, the fee caps at $1,000 USDC. By denominating in USDC rather than ETH, the fee is **deterministic and volatility-immune** — participants know the exact dollar cost before filing, with no dependency on price oracles or ETH market conditions. Compared to Upwork's flat $291 arbitration fee, this design is both more accessible for small disputes and more proportional for large ones. Frivolous disputes are further deterred by the combination of the scaled fee, the behavior bond, and reputation damage.

### 5.5 Reputation System

**Freelancer scoring:**

$$\text{score}_{\text{freelancer}} = \sum_{i} \left( V_i \times m_i \right) \div \left(1 + L \times 0.3 \right)$$

- $V_i$ = value of milestone $i$; $m_i$ = outcome multiplier (1.0 clean, 0.5 dispute win, 0.0 dispute loss); $L$ = disputes lost.
...
- $L$ = disputes lost; $C$ = jobs cancelled after freelancer selected; $A$ = milestones auto-approved via timeout.

**Graduated trust tiers (frontend-enforced):**

| Tier | Freelancer Threshold | Client Criteria | Behavior Bond Rate |
|------|---------------------|----------------|--------------------|
| New | 0 completed value | 0 completed value | 7.5% |
| Bronze | ≥ $1,000 | ≥ $1,000, completion ratio > 50% | 5% |
| Silver | ≥ $10,000 | ≥ $10,000, completion ratio > 75%, auto-approve < 20% | 2.5% |
| Gold | ≥ $50,000 | ≥ $50,000, completion ratio > 90%, auto-approve < 10% | 1% |
...
| **Milestone Manipulation** | Client structures milestones to get the most valuable deliverable last, then cancels. | **Minimum milestone percentage (10%)**: no single milestone can be less than 10% of total contract value. Additionally, cancellation in ACTIVE state requires **mutual consent** (see §3.6) — the client cannot unilaterally cancel after the freelancer has committed. If the freelancer refuses to consent, the client must use the dispute path, which carries dispute fees, potential bond forfeiture, and reputation penalties. |
...
| Attack | How It Works | Defense |
|--------|-------------|---------|
| **Abandonment** | Freelancer stakes deposit, locks client's funds, then never delivers. | **Freelancer deposit (5%)** is forfeited to the **platform treasury** if no submission by $T_{\text{deadline}}$. Client calls `claimAbandonment()` → escrow returned to client. |
