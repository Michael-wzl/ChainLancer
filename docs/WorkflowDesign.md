...
| **5** | Freelancer | `submitMilestone()` | Milestones may be submitted in any order — the freelancer is not required to complete them sequentially. The submission must be made **before the milestone's absolute deadline** (`block.timestamp <= ms.deadline`); attempting to submit after the deadline reverts. The `deliverableHash` is computed on the **plaintext** content (`deliverableHash = keccak256(plaintext)`) **before** encryption, ensuring the hash can be verified after decryption. The deliverable is then encrypted with $K_{job}$, uploaded to IPFS, and the CID is registered on-chain via `DataAvailability.sol` (platform pinning node auto-pins upon `CIDRegistered` event). The review timer ($T_{\text{review}}$, as chosen by the client at Step 1) starts counting from `submittedAt`. |
...
| **8** | Contract | Final release | After the last milestone is approved (or resolved via dispute): freelancer's deposit (graduated by tier) is refunded to their withdrawable balance; client's behavior bond (remaining after any dispute slashing) is refunded; `recordJobCompleted()` is called on `Reputation.sol` for the client (recording total value) and `recordFreelancerJobCompleted()` for the freelancer; data availability retention expiry is set to `block.timestamp + 21 days`. The milestone completion check considers `Approved`, `AutoApproved`, and `Resolved` (post-dispute) statuses as finalized. Note: `totalMilestoneCount` is **not** incremented at job completion — it is tracked per-milestone via `recordMilestoneResolved()` called on each individual milestone resolution (approval, auto-approval, or dispute resolution), ensuring the auto-approve rate denominator is always current throughout the job lifecycle. |
...
**6. Frontend-side integrity verification.** All IPFS content (agreements, deliverables, proposals) is verified after decryption by recomputing the content hash and comparing it against the on-chain value (`agreementHash`, `deliverableHash`, `proposalHash`). If the hash does not match — indicating tampered or corrupted IPFS content — the frontend displays an integrity warning and refuses to display the content as authoritative. This provides a client-side tamper-detection layer complementing the on-chain hash anchors.
...
| **APPLICATIONS** (freelancer selected, awaiting `confirmAndStake()`) | Client | `cancelJob()` | Client **cannot cancel while the freelancer has a live pending offer** (`block.timestamp <= selectedAt + T_STAKE`). The client must wait for the $T_{\text{stake}}$ window to expire before cancelling. Once the offer has expired: 100% of escrowed funds returned to client. Behavior bond refunded in full. | **No reputation penalty.** The freelancer implicitly declined by not staking within $T_{\text{stake}}$ — the client is not at fault for cancelling a job where no freelancer has committed. The selected freelancer's reputation is similarly unaffected. |
...
1. **Mutual cancellation is always available in ACTIVE state.** If both parties agree, the job can be cleanly wound down at any point. Already-released milestone funds are not clawed back — only remaining escrow is returned. When a cancellation request is pending, the counterparty's UI presents all available options (Accept / Decline / Ignore) with a clear explanation of consequences and a confirmation dialog — ensuring informed consent rather than a one-sided prompt.
...
| **2** | Both parties | `submitEvidence()` | Both parties have a **5-day evidence window** ($T_{\text{evidence}}$) to submit evidence. Evidence is **AES-encrypted with $K_{job}$** (using the same encryption pattern as agreements and deliverables) and uploaded to IPFS as an opaque binary blob; the CID is registered on-chain via `DataAvailability.sol` as `ContentType.Evidence` (platform pinning node auto-pins upon `CIDRegistered` event), and the ciphertext hash is recorded on-chain with a block timestamp. Since evidence CIDs are publicly discoverable on-chain via `Dispute.getEvidence()`, encryption ensures that dispute-sensitive business details are not exposed to third parties — only the client, freelancer, and assigned judge (who possess $K_{job}$) can decrypt. Submissions after the evidence deadline revert. Each party is capped at **20 evidence submissions** per dispute (`MAX_EVIDENCE_PER_PARTY`) to bound gas costs and prevent evidence-flooding attacks. After the evidence deadline passes, either party or the platform admin (`PLATFORM_ADMIN` role) calls `closeEvidencePhase()` to transition the dispute to the `AwaitingJudge` phase. (On-chain contracts cannot execute timed transitions automatically — an explicit transaction is required.) |
| **3** | Platform Admin | `assignJudge()` | A `PLATFORM_ADMIN` assigns a judge with relevant domain expertise. The `assignJudge()` function accepts the judge address and a **33-byte compressed ephemeral public key** $(pk_{\text{judge}}^{\text{eph}})$ specific to this dispute. The function first enforces a **conflict-of-interest check**: the judge address must not be the client or freelancer of the dispute — preventing accidental or malicious appointment of a dispute party as their own judge. The function then atomically: (a) records the judge address and ephemeral key on-chain, (b) **grants the `PLATFORM_JUDGE` role** to the judge via `AccessControl`, enabling them to call `submitRuling()`, (c) starts the key distribution timer ($T_{\text{keyDistribution}}$ = 2 days), and (d) transitions the dispute to the `KeyDistribution` phase. The ephemeral public key is emitted in a `JudgeAssigned(disputeId, judgeAddress, ephemeralPubKey)` event. This ensures that even if a judge's long-term identity key is compromised, no historical dispute content is exposed — each dispute's decryption capability is isolated to its ephemeral key. The admin must complete this assignment within **$T_{\text{judgeAssignment}}$ = 3 days** of the dispute entering the `AwaitingJudge` phase. If the deadline passes without a judge being assigned, either dispute party or a `PLATFORM_ADMIN` may call `claimJudgeAssignmentDefault()` to apply an automatic **Inconclusive** ruling (50/50 split, no deposit slash) and advance the dispute directly to the `Ruled` phase, ensuring disputes cannot be indefinitely stalled by administrative inaction. |
...
| **6** | Judge | `submitRuling()` | The judge submits a ruling on-chain during the `UnderReview` phase, before $T_{\text{ruling}}$ expires. The contract verifies `msg.sender == d.judge` (identity-based check rather than role-based, ensuring only the specifically assigned judge can rule on this dispute). The ruling is one of three outcomes (see §3.4). The contract enforces consistency constraints: if `FreelancerWins`, `freelancerShareBps` must be > 5000 (majority to freelancer); if `ClientWins`, `freelancerShareBps` must be < 5000 (majority to client); if `Inconclusive`, `freelancerShareBps` must be in the **balanced range [3000, 7000]** (30%–70%), preventing a nominally "inconclusive" ruling from producing a fully one-sided distribution; `depositSlashBps` cannot exceed 5000 (50% cap) and must be 0 for `Inconclusive` rulings. Before submission, the judge's reasoning text is **encrypted with $K_{job}$ and uploaded to IPFS** (CID registered via `DataAvailability.sol`), ensuring the full ruling rationale is preserved and auditable by authorized parties. The `reasoningHash = keccak256(reasoning)` is recorded on-chain for transparency and integrity verification. The dispute transitions to the `Ruled` phase. |
...
| **0 — Inconclusive** | Funds split per the judge's `freelancerShareBps` (**balanced range: 3000–7000**, i.e., 30%–70%), minus 2% protocol fee. Default is 50/50. The balanced range ensures that an "Inconclusive" ruling cannot produce a fully one-sided distribution — if the judge believes one party deserves more than 70%, they must use the `FreelancerWins` or `ClientWins` ruling instead, which carries the appropriate deposit and reputation consequences. | Both deposits untouched (the contract enforces `depositSlashBps == 0` for Inconclusive rulings). Dispute fee sent to the **platform treasury** (not refunded to either party). | No reputation penalty for either party. |
...
| **$T_{\text{judgeAssignment}}$** | 3 days (from dispute entering `AwaitingJudge` phase) | The platform admin must assign a judge within 3 days. If the deadline passes without assignment, either dispute party or a `PLATFORM_ADMIN` can call `claimJudgeAssignmentDefault()` to apply an automatic **Inconclusive** ruling (50/50 split, no deposit slash) and advance the dispute to the `Ruled` phase. If a judge is later removed via `claimRulingDefault()`, the 3-day deadline resets from that point. |
...
| **$T_{\text{ruling}}$** | 14 days (after both keys distributed, i.e., after dispute enters `UnderReview` phase) | Judge must submit ruling. If the judge fails to rule within $T_{\text{ruling}}$, **a dispute party, the judge, or a `PLATFORM_ADMIN`** can call `claimRulingDefault()` to revoke the failed judge's `PLATFORM_JUDGE` role, reset key submission state (both parties must re-distribute keys to the new judge), clear the judge and ephemeral public key, and transition the dispute back to `AwaitingJudge` for reassignment — at which point a fresh $T_{\text{judgeAssignment}}$ deadline begins. (Caller restriction prevents MEV bots from triggering default actions at deadline boundaries.) |

**Key non-cooperation handling via `claimKeyDefault()`** (callable only by dispute parties, the assigned judge, or a `PLATFORM_ADMIN` — preventing MEV front-running at deadline boundaries):
...
| **Tamper-proof verification** | $\text{agreementHash} = \text{keccak256}(\text{salt} \| \text{plaintext})$ on-chain; $\text{deliverableHash} = \text{keccak256}(\text{plaintext})$ on-chain; $\text{proposalHash} = \text{keccak256}(\text{proposalJSON})$ on-chain; the random 256-bit salt for agreements is embedded inside the encrypted IPFS payload | The frontend **automatically verifies integrity** after decryption: for agreements, it recomputes `keccak256(salt ‖ plaintext)` and compares against the on-chain `agreementHash`; for deliverables, it recomputes `keccak256(plaintext)` against the on-chain `deliverableHash`; for proposals, it recomputes `keccak256(proposalJSON)` against the on-chain `proposalHash`. If any check fails, an integrity warning is displayed. The judge performs the same verification during dispute review. The agreement salt prevents **confirmation attacks** — an adversary who suspects the agreement content cannot verify their guess by hashing candidate plaintexts against the on-chain hash, because the salt is unknown without $K_{job}$. |
...
- $L$ = disputes lost; $C$ = jobs cancelled after freelancer selected; $A$ = milestones auto-approved via timeout. The auto-approve rate for tier calculations is computed as `autoApproveCount / totalMilestoneCount`, where `totalMilestoneCount` is incremented **per-milestone** via `recordMilestoneResolved()` (called on each individual milestone approval, auto-approval, or dispute resolution) rather than in bulk at job completion. This ensures the auto-approve rate denominator is always current — even for partially-completed jobs with some milestones resolved and others still pending.
...
| Attack | How It Works | Defense |
| ------ | ------------ | ------- |
| **Judge Bribery** | A party bribes the platform-assigned judge. | This is a **known trade-off** of the centralized model. We mitigate it by: (1) the platform's business-level incentive to maintain neutrality, (2) ruling reasoning encrypted and uploaded to IPFS (hash on-chain for auditability), (3) the platform can assign a *panel* of judges for high-value disputes to reduce single-point corruption, (4) **ephemeral per-dispute keypairs** ensure that a compromised judge cannot retroactively access other disputes' content, (5) **conflict-of-interest check** — `assignJudge()` rejects appointments where the judge is a dispute party. In a future version, this can be upgraded to decentralized arbitration. |
...
| **Evidence Leakage** (unrelated parties reading task contents) | **Per-job symmetric encryption ($K_{job}$)**: all agreements, deliverables, evidence, and ruling reasoning are AES-256 encrypted before IPFS upload. Only the client, freelancer, and (during disputes) the assigned judge possess $K_{job}$. On-chain stores only ciphertext hashes. IPFS uploads are routed through a **backend proxy** (or Pinata Signed Upload URLs from a serverless function) rather than exposing the Pinata JWT in the client-side bundle — preventing unauthorized uploads, unpinning, or quota abuse. |
| **Key Non-Cooperation** (party refuses to share $K_{job}$ with judge) | **Timeout-based default ruling ($T_{\text{keyDistribution}}$ = 2 days)**: failure to distribute the key results in an automatic ruling against the non-cooperating party. Default-claim functions (`claimKeyDefault()`, `claimRulingDefault()`) are restricted to dispute parties, the judge, or a `PLATFORM_ADMIN` — preventing MEV bots from front-running cooperative transactions at deadline boundaries. Submitting an incorrect key is verifiable **off-chain** by the judge via `keccak256(salt ‖ decrypted) ≠ agreementHash`, where the salt is extracted from the decrypted payload — an incorrect key produces gibberish from which no valid salt can be recovered, failing the check deterministically. On-chain verification is not possible without exposing $K_{job}$ in plaintext; the judge signals non-cooperation through the ruling. |
